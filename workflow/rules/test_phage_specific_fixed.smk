...
rule test_phage_splitting:
    input:
        # This explicitly expects an absolute path to the input file
        protein_file = "{input_path}"
    output:
        split_dir = directory("test_phage_specific/results/split_proteins"),
        split_list = "test_phage_specific/results/split_list.txt"
    log:
        "test_phage_specific/results/logs/test_phage_splitting.log"
    conda:
...
rule test_phage_prediction:
    input:
        protein_file = "test_phage_specific/results/split_proteins/{phage_id}.faa"
    output:
        result_dir = directory("test_phage_specific/results/phacts_results/tmp/{phage_id}"),
        result = "test_phage_specific/results/phacts_results/tmp/{phage_id}/{phage_id}.phacts.out"
    log:
        "test_phage_specific/results/logs/phacts_prediction/{phage_id}.log"
    threads: 1
    conda:
...
rule test_phage_aggregate_results:
    input:
        ...
        predictions = lambda wildcards: expand(
            "test_phage_specific/results/phacts_results/tmp/{phage_id}/{phage_id}.phacts.out",
            phage_id=get_test_phage_ids()
        )
    output:
        predictions = "test_phage_specific/results/phacts_results/phacts_predictions_compiled.tsv"
    log:
        "test_phage_specific/results/logs/phacts_aggregate_results.log"
    conda:
        config["conda_envs"]["phacts"]
    shell:
        """
...
rule test_phage_specific_phacts:
    input:
        protein_file = "{input_path}",  # Pass the absolute path to the input file
        split_list = "test_phage_specific/results/split_list.txt",
        predictions = "test_phage_specific/results/phacts_results/phacts_predictions_compiled.tsv"
    output:
        report = "test_phage_specific/results/test_report.txt"
    run:
        import os
        
        # Count the number of split files
        split_dir = "test_phage_specific/results/split_proteins"
        split_count = len([f for f in os.listdir(split_dir) if f.endswith(".faa")])
        
        # Check the number of predictions
        pred_file = "test_phage_specific/results/phacts_results/phacts_predictions_compiled.tsv"
        ...
        with open(output.report, "w") as f:
            ...
            f.write(f"Input file: {input.protein_file}\n")
            ...
            f.write(f"test_phage_specific/results/logs/\n")